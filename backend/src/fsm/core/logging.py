"""Structured-logging configuration for a backend process.
...
Levels default to WARNING so routine runs stay quiet. The caller names the prefix its environment
variables carry: under prefix "APP_", APP_LOG_LEVEL sets the root level per environment and
APP_LOG_LEVELS raises individual modules as comma-separated pairs
(e.g. "myapp.workers=DEBUG,myapp.web=INFO").
"""
...
# uvicorn's own format and ignore the configured level. Rerouting them through the root handler
# makes every uvicorn record share one format and obey that level.
...
def configure_logging(*, env_prefix: str) -> None:
    """Route all loggers through a single structlog-formatted stderr handler.

    Reads {env_prefix}LOG_LEVEL (default WARNING) for the root level and {env_prefix}LOG_LEVELS
    for per-module overrides. Safe to call repeatedly.
    """
    ...
    root.setLevel(os.environ.get(f"{env_prefix}LOG_LEVEL", _DEFAULT_LEVEL).upper())

    for spec in os.environ.get(f"{env_prefix}LOG_LEVELS", "").split(","):
