...
function renderForm(opts: { onSkip?: () => void } = {}) {
  const refresh = vi.fn()
  const onSaved = vi.fn()
  render(
    <AuthContext.Provider
      value={{ auth: { status: 'authenticated', user: baseUser }, refresh }}
    >
      <ProfileForm user={baseUser} onSaved={onSaved} onSkip={opts.onSkip} />
    </AuthContext.Provider>,
  )
...
describe('ProfileForm', () => {
  ...
  it('submits only the fields the user changed', async () => {
    ...
    await userEvent.type(screen.getByLabelText(/phone/i), '+972-50-123')
    await userEvent.click(screen.getByRole('button', { name: /save/i }))

    expect(updateProfile).toHaveBeenCalledWith({ phone: '+972-50-123' })
